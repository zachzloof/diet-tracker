...
- `contributors.ts`: `contributionsTo(entries, key, limit)` ranks a day's entries by one nutrient or food group (largest first, each with its share of the day's total, the rest folded into one line). Behind the panels that open from Today's food-group rows and nutrient tiles.
- `weight.ts`: `weightTrend` (a trailing seven-day mean per weigh-in) and `weightRate` (kg per week between the first and last trend points, null under seven days of span). Pure; the Progress screen and recalibration both use it.
- `recalibration.ts`: `assessRecalibration(ctx)` (targets.md section 10) returns `not_due`, `not_enough_data`, `unavailable`, `on_track` or a `proposal` with the current and proposed energy and macros. The engine input `recalibrationKcal` carries the confirmed adjustment; `computeTargets` applies it after the pace, before the clamps and floor, and names it in the energy reason.
