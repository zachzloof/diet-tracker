...
| profiles | 2 | 1:1 with users (`user_id` is the primary key). sex, dob (date), height_cm, weight_kg, body_fat_pct?, goal, pace?, goal_weight_kg?, activity, training_type, training_days, diet_pattern, allergies text[], dislikes text[], timezone, units, flags jsonb, recalibration_snoozed_until date? (slice 5, migration 0005). Enums are text validated by zod. `weight_kg` is also updated by the newest weigh-in (D19) |
| target_versions | 2 | user_id, effective_from (date), trigger (onboarding, profile_change, recalibration), inputs jsonb (TargetInput, which may carry `recalibrationKcal` from slice 5), computed jsonb (Targets, no overrides), overrides jsonb (pins), effective jsonb (Targets with pins), explanation jsonb?, created_at. Latest (effective_from, created_at) wins; overrides are updated in place on that row; older rows are history. Recalibration inserts a row with the trigger `recalibration` (D20) |
| weight_entries | 2 | user_id, day, weight_kg, note?; unique (user_id, day). Written by profile saves that change weight and by `PUT /api/v1/weight` (slice 5); the recalibration check reads the last 14 days of them |
| ai_calls | 2 | user_id?, purpose, model, input_tokens?, output_tokens?, latency_ms, ok, error?, web_search_calls (default 0, migration 0003), created_at. One row per attempt; moved up from slice 3 because the plan explanation is the first call |
...
- Recalibration (`apps/api/src/progress/recalibration-service.ts`) reuses `scoreDays` for the 14-day window plus one range query on `weight_entries`; the decision is the pure `assessRecalibration`. Applying is one insert into `target_versions` and an update of the profile weight in the same transaction.
- Account deletion is `DELETE FROM users WHERE id = $1` and nothing else: every user-owned table cascades (slice 5 test `account.test.ts` checks eight tables). Export reads every table for the user in one `Promise.all` and validates the result with `accountExportSchema` before it leaves.
